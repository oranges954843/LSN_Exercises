...
void Restart(void) {
	...
	//initial configuration
	ifstream ReadConf("config.gibbs");
	for (int i=0; i<nspin; ++i) {
		ReadConf >> s[i];
	}
	ReadConf.close();
...
void Move() {
	int o;
	double prob_up, exp_up;

	o = (int)(rnd.Rannyu()*nspin);
	exp_up = Boltzmann(-1,o);
	if(turn_on == true){
		if(-1 != s[o])
			exp_up = exp_up * delta_ene[idown_h];
	}
	prob_up = 1 / (1.0 + exp_up); //probability of spin up
	if(rnd.Rannyu() < prob_up){
		s[o] = 1;
	}
	else {
		s[o] = -1;
...
double Boltzmann(double sm, int ip) {
	...
	if(nn_ene == 2) return delta_ene[iup];
	else if(nn_ene == 0) return 1;
	else return delta_ene[idown];
}
...
void Averages(int iblk) {
	...
	if(turn_on == false){
		...
		WriteEne.open("ene.gibbs",ios::app);
		WriteHeat.open("heat.gibbs",ios::app);
		WriteChi.open("chi.gibbs",ios::app);
		...
		WriteMag.open("mag.gibbs",ios::app);
...
void ConfFinal(void) {
	...
	if(turn_on == false){//Writing final results
		...
		WriteEne.open("eneVStemp.gibbs",ios::app);
		WriteHeat.open("heatVStemp.gibbs",ios::app);
		WriteChi.open("chiVStemp.gibbs",ios::app);
		...
		WriteMag.open("magVStemp.gibbs",ios::app);
