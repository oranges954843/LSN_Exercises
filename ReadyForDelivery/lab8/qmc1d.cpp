...
double variationalWaveFunction(double v){
	...
	//return 1.0;
	double a = v * mu / (2 * pow(sigma,2));
	double b = -(v*v + mu*mu)/(2*pow(sigma,2));
	return 2 * cosh(a) * exp(b);
}

double variationalWaveFunction_second(double v)
{
	//return 0;
	double a = v*v + mu*mu - sigma*sigma;
	double b = 2 * v * mu;
	double argo = v * mu / pow(sigma,2);
	return (a * cosh(argo) - b * sinh(argo)) / (pow(sigma,4) * cosh(argo));
}
